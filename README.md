Ekta Uniyal Portfolio

A responsive personal portfolio built with React, Framer Motion, and modern CSS.
It showcases my projects, about section, resume, and a contact form using EmailJS.

![Hero / Home Page]
![About Section]
![Resume Section]
![Projects Section]
![Contact Me]
![Email Sent]
![Email Received]



Features

Hero section with animated text and buttons

About section with animated social links (LinkedIn, GitHub, Instagram)

Resume section showcasing education & skills

Projects section with live and GitHub links

Contact form integrated with EmailJS

Fully responsive and mobile-friendly

Smooth animations using Framer Motion

How to Run Locally

Clone the repository:
git clone https://github.com/YOUR_GITHUB_USERNAME/portfolio.git

Navigate to the project folder:
cd portfolio

Install dependencies:
npm install

Add your EmailJS keys in a .env file:

REACT_APP_EMAILJS_SERVICE_ID=YOUR_SERVICE_ID
REACT_APP_EMAILJS_TEMPLATE_ID=YOUR_TEMPLATE_ID
REACT_APP_EMAILJS_USER_ID=YOUR_USER_ID


Start the development server:
npm start

Open http://localhost:3000
 to view the website.

Deployment

You can deploy this portfolio using Netlify, Vercel, or GitHub Pages.
Replace the EmailJS keys with your own for the contact form to function.

Give a Star & Credit

If you use or like this portfolio, please give it a ⭐ and credit my GitHub:ektauniyal123
